...
parser.add_argument('--ood_dataset', type=str, default="svhn", metavar='S',
...
def reweight_algo_v1(model, opt, train_batch_list, valid_batch_list, task_weights, loss, shots, query_num, ways, device):
    ...
    for i in range(len(valid_batch_list)):
        learner = model.clone()
        evaluation_error, evaluation_accuracy = fast_adapt(valid_batch_list[i],
                                                           learner,
                                                           loss,
                                                           1,
                                                           shots,
                                                           ...
                                                           device,
                                                           1)
        val_grads_list.append(torch.autograd.grad(evaluation_error, learner.parameters(), only_inputs=True))
    val_grads = [flatten_params(grad).reshape(1, -1) for grad in val_grads_list]
    val_grads = torch.cat(val_grads, dim=0)
    val_grads = val_grads.mean(dim=0).reshape(1, -1)
...
def main(
    ...
    if ood_data == 'svhn':
        pkl_filename = './' + str(CLUSTER_COUNT) + '_kmeans_org_svhn_model_1008.pkl'
        with open(pkl_filename, 'rb') as file:
            kmeans = pickle.load(file)
        pkl_filename = './' + str(CLUSTER_COUNT) + '_org_svhn_clusters_1008.pkl'
        with open(pkl_filename, 'rb') as file:
            cluster_labels = pickle.load(file)
