...
class HeuristicSinewave:
    ...
    def main(self):
        ...
        for iteration in range(self.iterations+1):
            ...
            task_weights = torch.from_numpy(np.random.rand(len(train_batch_tasks_list))).to(self.device)
        ...
        with open(r'maml_heuristic_sine_losses' + str(self.shots) + str(self.task_ood_lv) + '.txt', 'w') as csv_file:
            csv_file.writelines(str_losses)
        torch.save(self.meta_model, 'maml_heuristic_sine_model' + str(self.shots) + str(self.task_ood_lv) + '.pl')
