...
class ReWeightSinewave:
    ...
    def reweight_algo(self, model, opt, train_batch_list, valid_batch_list, loss, device):
        eps = torch.nn.Parameter(torch.zeros(len(train_batch_list), device=device), requires_grad=True)
        ...
        with higher.innerloop_ctx(model, opt) as (fmodel, diffopt):
            ...
            w_tilde = torch.clamp(-grad_all, min=0)
    ...
    def main(self):
        ...
        for iteration in range(self.iterations+1):
            ...
            task_weights = self.reweight_algo(maml_reweight, opt, train_batch_tasks_list, valid_batch_tasks_list, loss_func, self.device).detach()
            ...
            print('Meta-Re-Weighting Train Error', meta_train_error / meta_batch_size)
            print('Meta-Re-Weighting Valid Error', meta_valid_error / meta_batch_size)
            if iteration % 10 == 0:
                meta_test_error = 0.0
                meta_batch_size = self.tps
        ...
        with open(r'maml_rewt_sine_losses' + str(self.shots) + str(self.task_ood_lv) + '.txt', 'w') as csv_file:
            csv_file.writelines(str_losses)

        torch.save(self.meta_model, 'maml_rewt_sine_model' + str(self.shots) + str(self.task_ood_lv) + '.pl')