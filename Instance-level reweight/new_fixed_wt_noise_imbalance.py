...
def fast_adapt(batch, sample_indices, learner, loss, adaptation_steps, device, task_weight):
    ...
    for step in range(adaptation_steps):
        train_error = loss(learner(adaptation_data), adaptation_labels) @ task_weight[sample_indices[0]]
...
def generate_no_class_imb_indices(dataset, shots, num_tasks, ways):
    indices = []
    classes = []
...
def generate_noisy_data(dataset, noise_ratio):
    ...
    dataset.y[selected_lbls] = 63 - dataset.y[selected_lbls]
...
def main(
    ...
    train_classes, train_indices = generate_no_class_imb_indices(dataset=train_dataset, shots=train_shots, num_tasks=20000, ways=5)
    valid_classes, valid_indices = generate_no_class_imb_indices(dataset=valid_dataset, shots=valid_shots, num_tasks=2000, ways=5)
    test_classes, test_indices = generate_no_class_imb_indices(dataset=test_dataset, shots=test_shots, num_tasks=600, ways=5)
    # Create model
    ...
    task_weights = torch.ones(weight_count) * (1/weight_count)
