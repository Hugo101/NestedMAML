...
def reweight_algo_v1(model, opt, train_batch_list, valid_batch_list, task_weights, loss, device):
    ...
    for i in range(len(valid_batch_list)):
        learner = model.clone()
        valid_error, _ = fast_adapt_nored(valid_batch_list[i], learner, loss, 1, device)
        valid_error = valid_error.mean()
        grad_all = torch.autograd.grad(valid_error, learner.parameters(), only_inputs=True)
        val_grads_list.append(flatten_params(grad_all).reshape(1, -1))

    val_grads = torch.cat(val_grads_list, dim=0)
...
def main(
    ...
    for iteration in range(num_iterations + 1):
        ...
        grads_tensor = torch.clamp(grads_tensor, min=-1 * thres, max=1000).transpose(0, 1).reshape(-1)
