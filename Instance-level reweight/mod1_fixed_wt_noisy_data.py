...
def reweight_algo(model, opt, train_batch_list, valid_batch_list, sample_indices, task_weights, loss, device):
    ...
    with higher.innerloop_ctx(model, opt) as (fmodel, diffopt):
        ...
        for i in range(len(train_batch_list)):
            ...
            #adaptation_data, adaptation_labels = adaptation_data.to(device), adaptation_labels.to(device)
            evaluation_data, evaluation_labels = train_batch_list[i][1]
            evaluation_data, evaluation_labels = evaluation_data.to(device), evaluation_labels.to(device)
            #data = torch.cat((evaluation_data, adaptation_data), 0)
            #labels = torch.cat((adaptation_labels, evaluation_labels), 0)
            indices = []
            #indices.extend(sample_indices[i][0])
            indices.extend(sample_indices[i][1])
            outputs_hat = fmodel(evaluation_data)
            tmp_loss = loss(outputs_hat, evaluation_labels)
            forward_loss += (tmp_loss @ fmodel.eps[indices])
        diffopt.step(forward_loss)
        backward_loss = 0
        for i in range(len(valid_batch_list)):
            #support_data, support_labels = valid_batch_list[i][0]
            ...
            query_data, query_labels = valid_batch_list[i][1]
            query_data, query_labels = query_data.to(device), query_labels.to(device)
            outputs_hat = fmodel(query_data)
            backward_loss += (loss(outputs_hat, query_labels) @ (
                        torch.ones(len(query_data)).to(device) / len(query_data)))
...
def main(
    ...
    if cuda and torch.cuda.device_count():
        ...
        device = torch.device('cuda')
    # Create Datasets
    train_dataset = l2l.vision.datasets.MiniImagenet(root='~/data', mode='train', transform=transforms.Compose([
        transforms.ToPILImage(), transforms.ToTensor(), transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))]))
    train_dataset, noisy_indices = generate_noisy_data(train_dataset, noise_frac)
    # NO NOISY DATA
    valid_dataset = l2l.vision.datasets.MiniImagenet(root='~/data', mode='validation', transform=transforms.Compose([
        transforms.ToPILImage(), transforms.ToTensor(), transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))]))
    test_dataset = l2l.vision.datasets.MiniImagenet(root='~/data', mode='test', transform=transforms.Compose([
        transforms.ToPILImage(), transforms.ToTensor(), transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))]))
    ...
    task_weights = torch.ones(weight_count) * weight_init # * (1/weight_count)
    ...
    for iteration in range(num_iterations + 1):
        ...
        for idx in train_batch_indices:
            ...
            adaptation = []
            adaptation.append(torch.cat([train_dataset.dataset[x][0].view(1, train_dataset.dataset[x][0].shape[0], train_dataset.dataset[x][0].shape[1], train_dataset.dataset[x][0].shape[2]) for x in train_indices[idx][0]], dim=0).type(torch.float))
            adaptation.append(torch.Tensor([train_dataset.dataset[x][1] for x in train_indices[idx][0]]).to(device))
            adaptation[1] = label_conversion(adaptation[1])
            batch.append(adaptation)
            del adaptation
            support = []
            support.append(torch.cat([train_dataset.dataset[x][0].view(1, train_dataset.dataset[x][0].shape[0], train_dataset.dataset[x][0].shape[1], train_dataset.dataset[x][0].shape[2]) for x in train_indices[idx][1]], dim=0).type(torch.float))
            support.append(torch.Tensor([train_dataset.dataset[x][1] for x in train_indices[idx][1]]).to(device))
        ...
        for idx in valid_batch_indices:
            ...
            batch = []
            adaptation = []
            adaptation.append(torch.cat([valid_dataset.dataset[x][0].view(1, valid_dataset.dataset[x][0].shape[0],
                                        valid_dataset.dataset[x][0].shape[1], valid_dataset.dataset[x][0].shape[2])
                                         for x in valid_indices[idx][0]], dim=0).type(torch.float))
            adaptation.append(torch.Tensor([valid_dataset.dataset[x][1] for x in valid_indices[idx][0]]).to(device))
            adaptation[1] = label_conversion(adaptation[1])
            batch.append(adaptation)
            del adaptation
            support = []
            support.append(torch.cat([valid_dataset.dataset[x][0].view(1, valid_dataset.dataset[x][0].shape[0], valid_dataset.dataset[x][0].shape[1], valid_dataset.dataset[x][0].shape[2]) for x in valid_indices[idx][1]], dim=0).type(torch.float))
            support.append(torch.Tensor([valid_dataset.dataset[x][1] for x in valid_indices[idx][1]]).to(device))
            support[1] = label_conversion(support[1])
            batch.append(support)
            del support
        ...
        tmp_weights = task_weights.detach().clone()
        ...
        grads_tensor = torch.clamp(grads_tensor, min=-1 * thres, max=1000)
        #tmp_wts = torch.clamp(task_weights[train_batch_indices] - weight_lr * grads_tensor, min=0)
        tmp_wts = torch.clamp(task_weights - weight_lr * grads_tensor, min=0)
        tmp_wts[torch.where(tmp_wts==0)] = 1e-5
        #if (wt_loss + (torch.flatten(grads_tensor) @ torch.flatten((tmp_wts - task_weights[train_batch_indices])))) > 0:
        #    task_weights[train_batch_indices] = tmp_wts
        #task_weights[train_batch_indices] = tmp_wts
        task_weights = tmp_wts
        #task_batch_weights = task_weights[train_batch_indices]
        task_batch_weights = task_weights
        opt.zero_grad()
        ...
        noisy_weights = task_weights.detach().cpu().numpy()[noisy_indices].mean()
        non_noisy_indices = list(set(np.arange(len(train_dataset.dataset.y))).difference(set(noisy_indices)))
        non_noisy_weights = task_weights.detach().cpu().numpy()[non_noisy_indices].mean()
        ...
        if iteration % 100 == 0:
            ...
            for t in range(NUM_TEST_POINTS):
                ...
                batch = []
                adaptation = []
                adaptation.append(torch.cat([test_dataset.dataset[x][0] .view(1, test_dataset.dataset[x][0].shape[0], test_dataset.dataset[x][0].shape[1], test_dataset.dataset[x][0].shape[2]) for x in test_indices[t][0]], dim=0).type(torch.float))
                adaptation.append(
                    torch.Tensor([test_dataset.dataset[x][1] for x in test_indices[t][0]]).to(device))
                adaptation[1] = label_conversion(adaptation[1])
                batch.append(adaptation)
                del adaptation
                support = []
                support.append(torch.cat([test_dataset.dataset[x][0].view(1, test_dataset.dataset[x][0].shape[0], test_dataset.dataset[x][0].shape[1], test_dataset.dataset[x][0].shape[2]) for x in test_indices[t][1]], dim=0).type(torch.float))
                support.append(torch.Tensor([test_dataset.dataset[x][1] for x in test_indices[t][1]]).to(device))
                support[1] = label_conversion(support[1])
                batch.append(support)
                del support
                evaluation_error, evaluation_accuracy = fast_adapt(batch,
